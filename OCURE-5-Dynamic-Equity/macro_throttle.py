...
class MacroThrottle:
    ...
    """
    def __init__(self, m_min=0.25, tau=1.0, theta_on=0.1, theta_off=-0.2, smooth_span=10):
        self.m_min = m_min
        self.tau = tau
    ...
    def compute_throttle(self, btc_prices, spy_prices, hyg_prices=None, ief_prices=None, panel_prices=None):
        ...
        # 1. BTC/SPY Relative Distance Z-score
        ...
        z_btc = z_btc.fillna(0.0)

        scores = [z_btc]

        # 2. Credit Risk (HYG / IEF)
        if hyg_prices is not None and ief_prices is not None:
            ...
            safe_hyg = safe_hyg.reindex(safe_btc.index).ffill()
            safe_ief = safe_ief.reindex(safe_btc.index).ffill()
        ...
        if panel_prices is not None:
            ...
            safe_panel = safe_panel.reindex(safe_btc.index).ffill()
